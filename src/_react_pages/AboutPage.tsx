...
import desktopBanner from '../images/r-e-d-about-us-desktop-banner.webp';
import mobileBanner from '../images/r-e-d-about-us-mobile-banner.webp';
import teamImage from '../images/r-e-d-about-us-our-team.webp';
...
export default function AboutPage() {
  ...
  const [openPhilosophyId, setOpenPhilosophyId] = useState<string | null>(null);

  const coreValues: AccordionItem[] = [
  ...
  const philosophies: AccordionItem[] = [
  ...
  const toggleValue = (id: string) => {
    setOpenValueId(openValueId === id ? null : id);
  };

  const togglePhilosophy = (id: string) => {
    setOpenPhilosophyId(openPhilosophyId === id ? null : id);
  };

  return (
    <div className="w-full bg-[#f4f4f4] min-h-screen relative font-sans overflow-x-hidden pt-8">
      ...
      <div className="mx-auto lg:w-[95%] max-w-[1600px] px-8 md:px-0 lg:px-0 pb-20">

        {/* Back Breadcrumb */}
        <div className="mb-6 md:px-8 mt-4 md:mt-8">
          <a
            href="#"
        ...
        {/* Hero Section */}
        <div className="relative w-full rounded-3xl overflow-hidden shadow-sm mb-20 md:mb-32 h-[400px] md:h-[500px]">
          {/* Desktop Background */}
          <div
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
            <span className="font-poppins text-neutral-800 text-sm md:text-base font-medium mb-4">
              About us
            </span>
            <h1 className="font-poppins text-3xl sm:text-5xl md:text-[3.5rem] font-semibold text-neutral-800 leading-tight max-w-3xl">
              Tailored digital solutions <span className="text-[#00a5c5]">driving growth.</span>
            </h1>
          </div>
        </div>
        ...
        <div className="mb-20 md:mb-32">
          <h2 className="font-poppins text-3xl md:text-4xl font-semibold text-neutral-800 mb-6">
            Our story
          </h2>
          <p className="font-poppins text-neutral-700 text-sm md:text-base leading-relaxed max-w-full md:max-w-[95%]">
        ...
        {/* Our Team Section */}
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start mb-20 md:mb-32">
          <div className="w-full lg:w-1/2">
            <h2 className="font-poppins text-3xl md:text-4xl font-semibold text-neutral-800 mb-6">
              Our team
            </h2>
            <div className="space-y-6 font-poppins text-neutral-700 text-sm md:text-base leading-relaxed">
              <p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-20 md:mb-32">
          ...
          {/* Core Values Column */}
          <div>
            <div className="bg-[#00a5c5] text-white text-center py-4 rounded-xl mb-4">
              <h3 className="font-poppins text-2xl font-medium tracking-wide">Core values</h3>
            </div>
            <div className="space-y-2">
              {coreValues.map((item) => (
                <div key={item.id} className="bg-[#f0f0f0] border-b border-white rounded-lg overflow-hidden">
                  <button
                    onClick={() => toggleValue(item.id)}
                    className="w-full flex items-center justify-between px-6 py-4 text-left focus:outline-none"
                  >
                    <span className="font-poppins text-sm text-neutral-800 font-medium">{item.title}</span>
                    <ChevronDown className={`w-4 h-4 text-[#00a5c5] transition-transform duration-300 ${openValueId === item.id ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {openValueId === item.id && (
                      <motion.div
                      ...
                      >
                        <div className="px-6 pb-4 text-xs text-neutral-600 font-poppins">
                          {item.content}
                        </div>
                      </motion.div>
                    )}
          ...
          <div>
            <div className="bg-[#00a5c5] text-white text-center py-4 rounded-xl mb-4">
              <h3 className="font-poppins text-2xl font-medium tracking-wide">Our Philosophy</h3>
            </div>
            <div className="space-y-2">
              {philosophies.map((item) => (
                <div key={item.id} className="bg-[#f0f0f0] border-b border-white rounded-lg overflow-hidden">
                  ...
                  >
                    <span className="font-poppins text-sm text-neutral-800 font-medium">{item.title}</span>
                    <ChevronDown className={`w-4 h-4 text-[#00a5c5] transition-transform duration-300 ${openPhilosophyId === item.id ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {openPhilosophyId === item.id && (
                      <motion.div
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: 'auto', opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
        ...
        {/* CTA Banner Section */}
        <div
          className="w-full rounded-3xl overflow-hidden relative flex flex-col items-center justify-center text-center py-20 px-6 shadow-sm"
          style={{
            background: `linear-gradient(135deg, #e6e9f0 0%, #eef1f5 100%)` // Using a soft gradient similar to the image
          }}
        >
          <img loading="lazy" src={redLogo} alt="R-E-D Logo" className="h-8 mb-6" />
          <h2 className="font-poppins text-3xl md:text-4xl font-semibold text-neutral-800 mb-4">
            Ready to establish your<br />digital presences?
          </h2>
          <p className="font-poppins text-neutral-500 text-sm md:text-sm max-w-lg mb-8 leading-relaxed">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus eu ex ultricies, ornare velit vel, rhoncus metus.
          </p>
          <a
            href="#contact"
            className="bg-black hover:bg-[#00a5c5] text-white font-poppins text-sm font-semibold px-8 py-3.5 rounded-full transition-colors shadow-sm"
          >
